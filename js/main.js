...
$(document).ready(function () {
  ...
  $('.contact-form').submit((e) => {
    ...
    const button = $('#con_submit')

    if (!nameInput.val()) {
      response.addClass('error')
      response.text('Name is required');
      return
    }

    if (!emailInput.val()) {
      response.addClass('error')
      response.text('Email is required');
      return
    }

    if (!messageInput.val() && messageInput.val().length < 20) {
      response.addClass('error')
      response.text('Please submit a message that is at least 20 symbols');
      return
    }
    ...
    emailjs.send('service_0bsq9u7', 'template_47mou8q', templateParams).then(
      (res) => {
        response.addClass('success')
        response.text('Thank you for the message - we will come back to you soon!');
      },
      (error) => {
        response.addClass('error')
        response.text('Err, something went wrong ... please try again!');
      },
    ).catch((err) => {
      response.addClass('error')
      response.text('Err, something went wrong ... please try again!');
    }).finally(() => button.text('SUBMIT'));

    setTimeout(() => { response.text('') }, 10000)
  })
